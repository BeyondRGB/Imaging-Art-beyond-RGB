<script context="module">
	...
	export function autoSortImages(images, externalStack) {
		...
		// Otherwise, use the legacy sorting (not recommended)
			...
			imageStack.bestFlatFieldImages = [
				bestFlatFieldImage,
				maxBy(images, probabilityScoreProperties[2]),
			];
			...
			imageStack.bestDarkFieldImages = [
				bestDarkFieldImage,
				maxBy(images, probabilityScoreProperties[3]),
			];
			...
			// handle A - B sorting
			if (includeTarget) {
				imageStack.bestTargetImages = sortImageByLighting(
					imageStack?.bestTargetImages[0],
					imageStack?.bestTargetImages[1],
					externalStack.imageA[0][0]
				);
				externalStack.targetA = [imageStack?.bestTargetImages[0]];
				externalStack.targetB = [imageStack?.bestTargetImages[1]];
			}

			imageStack.bestFlatFieldImages = sortImageByLighting(
				imageStack?.bestFlatFieldImages[0],
				imageStack?.bestFlatFieldImages[1],
				externalStack.imageA[0][0]
			);
			externalStack.flatfieldA = [imageStack?.bestFlatFieldImages[0]];
			externalStack.flatfieldB = [imageStack?.bestFlatFieldImages[1]];

			imageStack.bestDarkFieldImages = sortImageByLighting(
				imageStack?.bestDarkFieldImages[0],
				imageStack?.bestDarkFieldImages[1],
				externalStack.imageA[0][0]
			);
		...
		// Otherwise, use the legacy sorting (not recommended)
			...
			each(Object.keys(artImagesByNumber), function (artImageSetKey) {
				...
				if (artImageSet.length == 2) {
					artImageSet = sortImageByLighting(
						artImageSet[0],
						artImageSet[1],
						externalStack.flatfieldA[0]
					);
