<script lang="ts">
		...
		overTracker = new OpenSeadragon.MouseTracker({
			...
			clickHandler: function (e) {
				if (viewer !== null) {
					...
					let pixelCoords = viewer.viewport.viewportToImageCoordinates(viewportPoint);
					trueShadowPos = {
						top: pixelCoords.y,
						left: pixelCoords.x,
					};
	...
	function updateSize() {
		try {
			...
			if (overlay !== null) {
				overlay.update(
					new OpenSeadragon.Rect(viewportPoint.x - size / 2, viewportPoint.y - size / 2, size, size)
				);
				overlay.drawHTML(viewer.overlaysContainer, viewer.viewport);
				trueSize = viewer.world.getItemAt(0).getContentSize().x * size;
			}
