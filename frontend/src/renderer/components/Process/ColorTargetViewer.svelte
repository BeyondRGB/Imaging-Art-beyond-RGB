<script lang="ts">
	...
	$: if ($processState.currentTab === 4 && viewer) {
		$processState.artStacks[0].colorTargetImage = { dataURL: "", filename: "" };
		closeViewer();
	}
	...
	function addOverlay(id) {
		...
		if (viewer && selBox && gridBox) {
			...
			const mo = new MutationObserver(() => {
				if (selBox.style.transform === "" || selBox.style.transformOrigin === "") {
			...
			trackers[id] = new OpenSeadragon.MouseTracker({
				...
				dragHandler: function (e) {
					...
					// that visualizes the math going on here.
					...
					const [cor_prev_x, cor_prev_y] = getBoxCenterOfRotation(box);
					...
					if (!pressPos.ele.classList.contains("corner")) {
						...
						const [cor_next_x, cor_next_y] = getBoxCenterOfRotation(box);
	...
	function closeViewer() {
		if (viewer) {
			viewer.close();
			viewerOpen = false;
			console.log("Color Target Viewer Closed");
		}
	}
						...
						{#each [...Array(target.rows * target.cols).keys()].map(i => i + 1) as boxIndex}
							<div
								...
								class:whitePoint={i !== 1 &&
									...
									target.whitePatch.row + (target.whitePatch.col - 1) * target.rows === boxIndex}
							>
								<div
									class:target={i === 0}
									class:verTarget={i === 1}
...
<style lang="postcss">
	...
	.line.whitePoint {
		background: radial-gradient(circle at center, transparent 80%, hsla(0, 0%, 100%, 1) 80%);
