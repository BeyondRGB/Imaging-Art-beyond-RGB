<script>
	...
	function resetTargets() {
		if (
			...
			Object.keys($processState.artStacks[0].colorTarget).length !== 0
		) {
			addDataToTarget();
		}
		if (
	...
	$: if (colorTarget?.whitePatch?.row != null && colorTarget.whitePatch.row > colorTarget.rows) {
		colorTarget.whitePatch.row = colorTarget.rows;
	} else if (colorTarget?.whitePatch?.row != null && colorTarget.whitePatch.row < 1) {
		colorTarget.whitePatch.row = 1;
	}

	// Clamp the white patch columns to the number of columns in the target
	$: if (colorTarget?.whitePatch?.col != null && colorTarget.whitePatch.col > colorTarget.cols) {
		colorTarget.whitePatch.col = colorTarget.cols;
	} else if (colorTarget?.whitePatch?.col != null && colorTarget.whitePatch.col < 1) {
		...
		<div class="left-content">
			<div class="image-container">
				...
				<ColorTargetViewer
					...
					bind:loading
					bind:viewerOpen
					bind:calibrationTargetRotationAngle
			...
			<ScrollContainer maxHeight="none" className="scroll-content">
				{#if viewerOpen}
					{#each [...targetArray, "Add"] as target, i (target)}
						...
						{:else if target !== "Add"}
							<Card
								className="card {i === 0 ? 'colorTarget' : ''} {i !== 0
									? 'verificationTarget'
									: ''}"
								variant="default"
								padding="none"
							...
							>
								<div class="card-content">
									<h2>{target.name}</h2>
									<input type="range" class="colorSlider" bind:value={target.color} max="360" />
									...
									<div class="refDataDiv">
										<span class="validatedTitle">
											{#if target.refData.fileName === "---None---.csv"}
												<span class="invalid"><AlertTriangleIcon size="1.5x" /> </span>
											{/if}
											Reference Data:</span
										>
									...
									<div class="sizeDiv">
										...
										<input type="range" bind:value={target.size} min=".3" max=".7" step=".01" />
									</div>
									{#if i !== 1}
										<div class="break" />
									...
									<button
										class="close"
										disabled={i === 0 && typeof verifyTarget != "undefined" && verifyTarget != null}
										on:click={() => removeTarget(i)}><XIcon size="1.25x" /></button
									>
								</div>
							</Card>
						{/if}
