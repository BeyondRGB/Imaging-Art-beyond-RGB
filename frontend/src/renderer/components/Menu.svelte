<script lang="ts">
	...
	import { currentPage, modal, appSettings, connectionState, connect } from "@util/stores";
	...
	$: if ($connectionState === "Closed" || $connectionState === "Closed - Error") {
		toast.push(
			"An error has occurred. Click the red icon to attempt to reconnect, or restart the program."
		);
	}
</script>
		...
		<div class="menuBtns">
			{#each Object.keys(routes).map(key => [key, routes[key]]) as item, i}
				{#if item[1].isShown && !item[1].default}
					...
					>
						<svelte:component this={item[1].icon} size="1.75x" class="menuNavIcon" />
