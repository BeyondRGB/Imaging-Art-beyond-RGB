...
// NOTE: using these will not work with a variable passed in, Only used with
// hard coded numbers
...
enum ResultType { CALIBRATION, GENERAL, VERIFICATION };

enum TargetType { GENERAL_TARGET, VERIFICATION_TARGET };
...
class ArtObject {
  ...
  public:
    ArtObject(std::string ref_file, IlluminantType ilumination,
              ObserverType observer, std::string output_directory, bool batch);
    ~ArtObject();

    RefData *get_refrence_data(
        btrgb::TargetType target_type = btrgb::TargetType::GENERAL_TARGET);
    ...
    std::unique_ptr<ColorTarget> get_target_pointer(std::string imageName,
                                                    TargetType type);
    ...
    void outputImageAs(enum output_type filetype, std::string name,
                       std::string filename = "");
    ...
    std::unordered_map<std::string, Image *>::iterator begin() noexcept {
        return images.begin();
    };
    std::unordered_map<std::string, Image *>::iterator end() noexcept {
        return images.end();
    };
};
...
class ArtObj_ImageAlreadyExists : public ArtObjectError {
  public:
    virtual char const *what() const noexcept {
        return "ArtObject Error: An image with that name already exists.";
    }
};
...
class ArtObj_ImageDoesNotExist : public ArtObjectError {
  public:
    virtual char const *what() const noexcept {
        return "ArtObject Error: That image does not exist.";
    }
};
...
class ArtObj_FailedToWriteImage : public ArtObjectError {
  public:
    virtual char const *what() const noexcept {
        return "ArtObject Error: Failed to write image.";
    }
};
...
class ArtObj_VerificationDataNull : public ArtObjectError {
  public:
    virtual char const *what() const noexcept {
        return "ArtObject Error: No Verification Data Exists.";
    }
};
