...
class Pipeline : public BackendProcess {
    ...
    const std::string key_map[11] = {
        "art",         "white",          "dark",
        "images",      "refData",        "standardObserver",
        "illuminants", "targetLocation", "verificationLocation",
  ...
  private:
    ...
    */
     ...
     * This will populate the art object with TargetData and the initial images
     * it will contain
     *
     ...
     *
     * @param art_obj the ArtObject that contains the General Info results
     * object.
     */
    void init_general_info(btrgb::ArtObject *art_obj);

    /**
     * @brief Get the illuminant type object from the request data provided by
     * frontend
     *
    ...
    /**
     * @brief Get the observer type object from the request data provided by
     * fronend
     *
     * @return ObserverType
     */
     ...
     * This will uses the output directory from the request data provided by the
     * frontend as a base and create a new folder within that directory in the
     * form BTRGB_<DATE>_<TIME> It then returns the new dir as a string.
     *
     * @return std::string
     */
