...
class SpectralCalibrator : public LeafComponent {
  public:
     ...
     *
     * @param func TODO this should be changed to the ComsObj once merged with
  ...
  private:
    ...
    cv::Mat R_camera;    // Container for holding the R_camera values that this
                         // process creates

    /**
     * @brief Initialize the starting input_array and M_refl
     * The MinProblemSolver needs the InputArray to be a 1d Matrix but we need
     * to optimize M_refl This function creates M_refl and fills with initial
     * values It then creates a 1d representation (input_array) pointing to the
     * values held in M_refl If values in either are modified they are modified
     * in both
     *
...
/**
 * @brief Helper class that defines the function used for optimizing R_refl to
 * minimize Z Z is based off of minimizing the difference between values
 * calculated using R_ref and the ground trueth values of the RefData
 *
 * An instance of this class gets passed to the MinProblemSolver which calls
 * calc during optimization
 *
 * NOTE: all member variables used in optimization are passed by reference on
 * construction. This means that the creator of this class already has the
 * results for R_refl and R_camera at the time that optimization complets
 *
 */
class WeightedErrorFunction : public cv::MinProblemSolver::Function {
  ...
  public:
     ...
     * @return double the Z value computed using current input vlaue. This is
     * what is beeing minimized.
     */
  ...
  private:
     ...
     *      sum += (difference between RefData dn R_camra) rased to the power of
     * 2 RMSE  = squar root of the sum
     *
     * @return double RMSE
     */
     ...
     *      e2 += (difference between max RefData value and max R_camera value)
     * rased to the power of 2 e2 = the sum for all wavelengths
     *
     * @return double
     */
     ...
     *  for every wavelength(row) in RefData/R_camera(actual signals)
     *      e3 += (difference between min RefData value and min R_camera value)
     * rased to the poer of 2 e3 = the sum for all wavelengths
     *
     * @return double
     */
