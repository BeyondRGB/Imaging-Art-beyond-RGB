...
/**
 * @brief This class is one of the ImagProcessingComponents and is the last step
 * in the pipeline It is responsible for outputing results of the calibration
 * process including outputing the final image and any/all results reported
 *
 */
class ResultsProcessor : public LeafComponent {
  ...
  public:
    enum FormatType { GEN_INFO, M_COLOR, M_SPECTRAL, COLORIMETRY, R_CAMERA };
  ...
  private:
     ...
     * This fill contains any and all results from the entire process and the
     * resulting file can be used to recreate the CalibrationResult objects
     *
     * @param images the ArtObject containing the results
     */
    ...
    /**
     * @brief This will write any image file still remaining in the ArtObject to
     * a .TIFF
     *
    ...
    void write_formated_results(std::string file_name, FormatType format_type,
