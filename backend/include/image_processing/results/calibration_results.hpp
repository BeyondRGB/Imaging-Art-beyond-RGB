 ...
 * This class can serialize itself into a json object and deserialize itself
 * given a json object This class can store result data in the forms of
 * String,Int,Double,cv::Mat
 *
 */
class CalibrationResults : public Jsonafiable, private CSVParser {

  public:
    enum ResultType { INT, DOUBLE, MATRIX };
    /**
     * Maps ResultsMatrix enum values to a string
     */
    const std::string type_key_map[3] = {"int", "double", "matrix"};
     ...
     * This writes to a .csv format with a single space between each result
     * written result format <ResultName> <ResultMetaData> <ResultValue/Values>
     * // Note: if the result is a matrix there will be additional lines
     * following for each row
     *
     ...
     * This serializes the class and adds all results contained in this class to
     * a json object
     *
     ...
     * This de-serializes the class and reconstructs a previouse instance of
     * this class from the given json
     *
     * NOTE: if data is missing or currupt in the given json it is posible that
     * some or all result info does not get reconstructed
     *
     ...
     * NOTE: if any data is missing or corrupt for a matix found in json that
     * matix will be skiped
     *
     * @param parser
     */
     ...
     * NOTE: if any data is missing or corrup for a double found in the json
     * that value will be skiped
     *
     * @param parser
     */
     ...
     * NOTE: if any data is missing or corrup for a int found in the json that
     * value will be skiped
     *
     * @param parser
     */
     ...
     * NOTE: if any data is missing or corrup for a string found in the json
     * that value will be skiped
     *
     * @param parser
     */
    void reconstruct_strings(
        Json parser); /**
                       ...
                       * If a name is given the first line will be the matrix
                       * name. If no name is given the first line will be the
                       * first row of the matix
                       *
                       * Each row of the matrix is writen to its own line with
                       * each value seperated by commas
                       *
                       * @param output_stream the stream to write to
                       * @param matrix the matrix to write
                       * @param matrix_name name of the matrix defaults to empty
                       * string
                       */
  ...
  private:
    ...
    /**
     * @brief Itterates over every matix stored and appends its output to the
     * given output_stream
     *
     * @param output_stream
     */
    ...
    /**
     * @brief Itterates over every stored integer and appends its output to the
     * given outpu_steam
     *
     * @param output_stream
     */
    ...
    /**
     * @brief Itterates over every stored double and append its output the given
     * output_stream
     *
     * @param output_stream
     */
    ...
    /**
     * @brief identifies what type of data is held in the given matrix and how
     * to extract it. Once value is retrieved it is written to the given
     * output_stream
     *
     * @param output_stream the stream to write to
     ...
     */
    void write_matrix_value(std::ostream &output_stream, cv::Mat matrix,
                            int row, int col);
};
