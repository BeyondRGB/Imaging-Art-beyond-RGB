...
class RefData : public CSVParser {
    const std::string ref_files[REF_COUNT] = {
        "APT_Reflectance_Data.csv", "CC_Classic_Reflectance_Data.csv",
        "CCSG_Reflectance_Data.csv", "NGT_Reflectance_Data.csv"};
  ...
  public:
    ...
    /**
     * Finds the white ColorPatch as defined by the reference data. The whit
     * patch is the one with the hightest y value. NOTE: this is not the same as
     * the user defined white patch
     ...
     * Y Value,<White ColorPatch yvalue>,Row,<White ColorPatch row>,Col,<White
     * ColorPatch col>
     */
    void output_xyz();
     ...
     * 	A1_wavlen380, B1_wavlen380, C1_wavlen380, ..., K1_wavlen380,
     * A2_wavlen380, ... , Kk_wavlen380 A1_wavlen390, B1_wavlen390,
     * C1_wavlen390, ..., K1_wavlen390, A2_wavlen390, ... , Kk_wavlen390
     *  ...			, ...		  , ...			, ...,
     * ...		   , ...		 , ... , ... A1_wavlen730,
     * B1_wavlen730, C1_wavlen730, ..., K1_wavlen730, A2_wavlen730, ... ,
     * Kk_wavlen730
     *
     *
     * @return cv::Mat
...
class RefData_FailedToRead : public RefDataError {
  public:
    ...
    RefData_FailedToRead(std::string file) {
        this->error = "RefData Error: failed to read " + file;
    }
...
class RefData_ParssingError : public RefDataError {
  public:
    ...
    RefData_ParssingError(std::string msg) {
        this->error = "RefData Parsing Error: invalid file format -> " + msg;
    }
    virtual char const *what() const noexcept { return error.c_str(); }
