 ...
 * An instance of the class gets created when a new msg is recieved from the
 * forntedn. It can then be passed on to a process so that the process can send
 * mesages as needed
 */
class CommunicationObj {
  ...
  public:
    CommunicationObj() {};
    CommunicationObj(server *s, websocketpp::connection_hdl hd1,
                     message_ptr msg);
    /**
     * Copy Constructor
     */
    ...
    void send_response(const std::string &responseType,
    ...
    void send_error(std::string msg, std::string sender,
                    cpptrace::stacktrace trace, bool critical = true);
    /**
    ...
    void send_spectrum_measured(float *estimated_data, float *referenced_data,
                                int size);
    ...
    void send_base64(std::string name, std::vector<uchar> *direct_binary,
                     enum btrgb::output_type type);

    void send_binary(std::string name, std::vector<uchar> *direct_binary,
                     enum btrgb::output_type type);
