...
class Json {
  ...
  public:
    enum Type { ANNY, ARRAY, BOOL, NUMBER, NONE, OBJECT, STRING };
    ...
    */
    ...
    template <typename T> T value_at(int index) {
        validate_is_array();
        ...
        if (!this->is_type<T>(item)) {
            std::string error =
                "Item at index(" + std::to_string(index) + ") is not a bool";
            throw std::exception();
        }
        return item.as<T>();
    }
  ...
  private:
    ...
    /**
     * Validates that the given index is within the bounds of the current Json
     * Array throws ParsingError if not
     */
    void validate_bounds(int index);

    template <typename T> bool is_type(jsoncons::json item) {
