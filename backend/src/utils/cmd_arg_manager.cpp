...
bool CMDArgManager::pars_optional_arg(std::string arg,
...
void CMDArgManager::handle_other(std::string arg) {
    if (arg == "-usage" || arg == "-u") {
        std::string usage_str =
            "usage: [options]\n"
            "\toptions:\n"
            "\t --test_run=<bool>: set true to bypass server and run "
            "testfunc() in main.cpp, this defaults to false\n"
            "\t --port=<int>: The local network port for frontend/backend "
            "communication.\n"
            "\t --app_root=<path>: set path for where applications resource "
            "folder can be found\n";
