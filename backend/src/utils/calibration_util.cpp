...
cv::Mat btrgb::calibration::build_target_avg_matrix(ColorTarget targets[],
                                                    int target_count,
                                                    int channel_count) {
    ...
    //  There should be a row for each channel of each target(ie 2 targes, 3
    //  channels = 6 rows) Each row should hold a number of values equle to the
    //  total number of ColorPatches of one ColorTarget
    ...
    cv::Mat color_patch_avgs =
        cv::Mat_<double>(mat_row_count, mat_col_count, CV_32FC1);
        ...
        for (int chan = 0; chan < channel_count; chan++) {
            ...
            for (int target_row = 0; target_row < row_count; target_row++) {
                for (int target_col = 0; target_col < col_count; target_col++) {
                    ...
                    float avg =
                        target.get_patch_avg(target_row, target_col, chan);
                    // Stroe avg value
...
cv::Mat btrgb::calibration::build_camra_signals_matrix(Image *art[],
                                                       int art_count,
                                                       int channel_count,
                                                       cv::Mat *offsets) {
    ...
    // each channel
    cv::Mat camra_sigs =
        cv::Mat_<double>(channel_count, height * width, CV_32FC1);
    ...
    for (int art_i = 0; art_i < art_count; art_i++) {
        ...
        for (int chan = 0; chan < chan_count; chan++) {
            ...
            for (int row = 0; row < art_c->height(); row++) {
                ...
                // The offset value subracted from each pixel, but we only need
                // to subtract offsets if some were given. Offsets default to
                // null
                ...
                for (int col = 0; col < art_c->width(); col++) {
                    ...
                    double px_val =
                        (double)art_c->getPixel(row, col, chan) - offset_value;
                    // The col in the six_chan matrix we ar currently adding
                    // values to
...
cv::Mat btrgb::calibration::apply_offsets(cv::Mat camera_sigs,
                                          cv::Mat offsets) {
...
void btrgb::calibration::display_matrix(cv::Mat *matrix, std::string name) {
    ...
    std::cout << name << " rows: " << matrix->rows << " " << name
...
btrgb::Image *btrgb::calibration::camera_sigs_2_image(cv::Mat camera_sigs,
                                                      int height) {
...
cv::Mat btrgb::calibration::image_2_camera_sigs(btrgb::Image *image, int height,
                                                int width) {
...
void btrgb::calibration::fill_Lab_values(cv::Mat *L_camera, cv::Mat *a_camera,
                                         cv::Mat *b_camera, cv::Mat *L_ref,
                                         cv::Mat *a_ref, cv::Mat *b_ref,
    ...
    *b_ref = cv::Mat_<double>(row_count, col_count, CV_64FC1);
    ...
    for (int row = 0; row < row_count; row++) {
        for (int col = 0; col < col_count; col++) {
            ...
            // Scale each by 100 because everything in xyz is between 0-1 and we
            // need to match the scale of the RefData
...
double btrgb::calibration::compute_deltaE_sum(RefData *ref_data, cv::Mat xyz,
    ...
    // delta E is the difference in color between the RefData and the actual
    // image Target(xyz Mat)
    WhitePoints *wp = ref_data->get_white_pts();
    double deltaE_sum = 0;
    for (int row = 0; row < row_count; row++) {
        for (int col = 0; col < col_count; col++) {
            ...
            // Store value in matrix. This matrix will hold the actual deltaE
            // values for each patch for the min avg found
...
double
btrgb::calibration::compute_90th_percentile(const cv::Mat &deltaE_values) {
    ...
    if (values.empty()) {
        throw std::runtime_error(
            "DeltaE matrix is empty, cannot compute 90th percentile.");
    }
...
double btrgb::calibration::compute_RMSE(cv::Mat R_camera, cv::Mat R_ref) {
    ...
    // N should be equivilent to 36 ie. the number of wavelengths ie. the number
    // of rows in R_camera/R_ref
    int N = R_camera.rows;
