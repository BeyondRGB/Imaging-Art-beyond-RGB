...
void TimeTracker::end_timeing(){
    this->end = time(0);
}

int TimeTracker::get_elapsed_sec(){
    return this->end - this->initial;
}
...
int TimeTracker::sec2ms(int sec){
    return sec * 1000;
}

float TimeTracker::sec2min(int sec){
    return sec / 60.0f;
}