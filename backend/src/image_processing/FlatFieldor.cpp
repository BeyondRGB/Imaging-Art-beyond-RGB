...
void FlatFieldor::execute(CommunicationObj *comms, btrgb::ArtObject *images) {
    ...
    try {
        ...
        try {
            ...
            comms->send_info(
                "ArtObject Image does not exist. Using embedded targets.",
                this->get_name());
            target_found = false;
        }
    ...
    // Set up variables for the overall size of all the images, they are all the
    // same size
    ...
    float whiteAvg =
        images->get_target("white1", btrgb::TargetType::GENERAL_TARGET)
            .get_patch_avg(whiteRow, whiteCol, 1);
    ...
    cv::GaussianBlur(copy, post_lowpass, cv::Size(3, 3), 2);
    ...
    if (target_found) {
        ...
        std::unique_ptr<btrgb::Image> target1copy(
        ...
        // Copy and delete instantly after operation
        std::unique_ptr<btrgb::Image> target2copy(
            new btrgb::Image("target2copy"));
        cv::Mat tcopy2 =
 ...
 * @param pAvg: Average Pixel value of the second channel of the overall art
 * image
 * @param wAvg: Average Pixel value of the second channel of the overall white
 * image
