...
BitDepthScaler::BitDepthScaler() : LeafComponent("Rescaling") {}
BitDepthScaler::~BitDepthScaler() {}

void BitDepthScaler::execute(CommunicationObj *comms,
    ...
    for (const auto &[key, im] : *images) {
        ...
        out3 << "Scaling \"" << im->getName() << "\" from " << raw_bd
             << " to 16 bits...";

        /* If the bit depth is invalid or already 16 bits: skip, do not scale
         * anything. */
