        ...
        try {
            ...
            if (path.empty()) {
                throw std::runtime_error(
                    "No file found with prefix: " + prefix +
                    ". Ensure all csv files generated by BeyondRGB are in the "
                    "same folder as the BTRGB file.");
            }
            ...
            if (!loadMatricesFromText(file_content)) {
                ...
                throw std::runtime_error(
                    ...
                    "Ensure all csv files generated by BeyondRGB are in the "
                    "same folder as the BTRGB file. Edited CSVs can not be "
                    "read.");
            }
            this->deltaE_values =
...
/***
 * Convert the six channels in art1 and art2 into a ColorManaged RGB image
...
void ColorManagedCalibrator::update_image(btrgb::ArtObject *images) {
    ...
    // Initialize 6xN Matrix to represent our 6-channel image
    // Each row represents a single channel, and N is the total pixels for
    // each channel
