...
void ColorManagedCalibrator::execute(CommunicationObj *comms,
    ...
    if (targetsFound or images->get_batch() == false) {
        ...
        try {
            ...
            if (path.empty()) {
                throw std::runtime_error("No file found with prefix: " + prefix + ". Ensure all csv files generated by BeyondRGB are in the same folder as the BTRGB file.");
            }

            std::ifstream input_file(path);
            if (!input_file.is_open()) {
                throw std::runtime_error("Unable to open file: " + path + ". Ensure all csv files generated by BeyondRGB are in the same folder as the BTRGB file.");
            }
            this->build_input_matrix();
            ...
            if (!loadMatricesFromText(file_content)) {
                ...
                throw std::runtime_error("Precalculated M and offset matrices could not be loaded. Ensure all csv files generated by BeyondRGB are in the same folder as the BTRGB file. Edited CSVs can not be read.");
            }
            this->deltaE_values =
...
double DeltaEFunction::calc_noise() const {
    ...
    // every delta has its own const param
    ...
    std::cout << "MAT GIVEN\n" << *this->M << std::endl;
    // TODO: figure out which index 0 is breaking this
    cv::Mat_<double> inverse_sums = (cv::Mat_<double>(1, 3) <<
        (1 / cv::sum(this->M->row(1))[0]),
        (1 / cv::sum(this->M->row(2))[0]),
        (1 / cv::sum(this->M->row(3))[0])
    );
    std::cout << "INVERSE SUMS FOUND\n" << inverse_sums << std::endl;
    ...
    for (int i = 1; i <= 3; i++)
    {
        ...
        {
            // rescale m i,j
            Mbar.at<double>(i, j) *= inverse_sums.at<double>(1, i);
        }
    }

    // calc deltaL
    for (int j = 1; j <= 6; j++)
        deltaL += std::pow(Mbar.at<double>(2, j), 2);
    ...
    // calc delta_beta
    for (int j = 1; j <= 6; j++)
        delta_alpha += std::pow(Mbar.at<double>(2, j) - Mbar.at<double>(3, j), 2);
