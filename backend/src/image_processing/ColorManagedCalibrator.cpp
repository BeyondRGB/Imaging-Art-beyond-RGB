...
void ColorManagedCalibrator::execute(CommunicationObj *comms,
    ...
    if (targetsFound or images->get_batch() == false) {
        ...
        this->deltaE_values = cv::Mat_<double>(
            target1.get_row_count(), target1.get_col_count(), CV_32FC1);
        ...
        try {
            ...
            this->deltaE_values =
                cv::Mat_<double>(this->ref_data->get_row_count(),
                                 this->ref_data->get_col_count(), CV_32FC1);
    ...
    // Use M and Offsets to convert the 6 channel image to a 3 channel
    // ColorManaged image, for both art and target
    std::cout << "Converting 6 channels to ColorManaged RGB image."
              << std::endl;
    try {
    ...
    // Dont remove art1 and art2 from the ArtObject yet as they are still needed
    // for spectral calibration
}

/**
 * @brief Sets up and runs the MinProblemSolver to optimize M and offsets for
 * min deltaE
 *
 */
void ColorManagedCalibrator::find_optimization() {
    ...
    // This allows for access to all results (M, offset, deltaE_values) once
    // optimization is complete
    cv::Ptr<cv::MinProblemSolver::Function> ptr_F(new DeltaEFunction(
        &this->optimization_input, // InputArray, contans values for both M and
                                   // offset
        &this->color_patch_avgs,   // Average values for all color patches
        &this->offest, // A Croping of InputArray whos view is just the offset
                       // values
        &this->M, // A croping of InputArray whose view is a 2d(3x6) matrix of M
                  // values
        this->ref_data, //
        &this->deltaE_values));
    ...
    cv::Mat step =
        (cv::Mat_<double>(1, 24) << 0.75, 0.75, 0.75, 0.75, 0.75, 0.75, // M
    ...
    min_solver->setTermCriteria(cv::TermCriteria(
        cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 50000, 1e-10));
...
void ColorManagedCalibrator::update_image(btrgb::ArtObject *images) {
    ...
    // Each row represents a single channel and N is the number total pixles for
    // each channel
    cv::Mat camra_sigs = btrgb::calibration::build_camra_signals_matrix(
        art, 2, 6, &this->offest);
...
void ColorManagedCalibrator::output_report_data(btrgb::ArtObject *images) {
    ...
    cv::Mat offset_avg =
        btrgb::calibration::apply_offsets(this->color_patch_avgs, this->offest);
    ...
    btrgb::calibration::fill_Lab_values(&L_camera, &a_camera, &b_camera, &L_ref,
                                        &a_ref, &b_ref, cm_xyz, this->ref_data);
...
void ColorManagedCalibrator::build_input_matrix() {
     ...
     *
     * All values will be stored in the InputArray and Matrix Headers will be
     * created for M, and Offset Thes headers will point to the values in the
     * InputArray and will simply represent that data in a different format An
     * change in the this->optimazation_input will be represented in M and
     * Offset
     */
    ...
    this->optimization_input = (cv::Mat_<double>(1, item_count) <<
                                    /*M*/ 0.1,
                                0.1, 0.25, 0.5, 0.1, 0.1,
                                /*M*/ 0.1, 0.1, 0.25, 0.1, 1.0, 0.1,
                                /*M*/ 0.1, 0.1, 0.25, 0.1, 0.1, 0.5,
                                /*Offset*/ 0.01, 0.01, 0.01, 0.01, 0.01, 0.01);

    // Create Matrix Header to Represents the 1d InputArray as a 2d Matrix for
    // easy extraction of M and offset
    cv::Mat opt_as_2d = cv::Mat(this->optimization_input).reshape(0, row_count);
    // Create Matrix Header to represent the 2d Matix M that points to the
    // values that are in the InputArray
    this->M = opt_as_2d(cv::Rect(0, 0, col_count, row_count - 1));
    // Create Matrix Header to represent the 1d Matrix offset that points to the
    // values that are in the InputArray
...
DeltaEFunction::DeltaEFunction(cv::Mat *opt_in, cv::Mat *cp_avgs,
                               cv::Mat *offeset, cv::Mat *M, RefData *ref_data,
                               cv::Mat *delE_values) {
    ...
    // ColorManagedCalibrator This means that once optimization is complete
    // ColorManagedCalibrator already has the resulting values Se doc strings in
    // ColorManagedCalibrator for details on each of these matracies
    this->opt_in = opt_in;
...
bool ColorManagedCalibrator::loadMatricesFromText(
    ...
    if (!std::getline(iss, line) ||
        line.find("M_color_offset") == std::string::npos) {
        return false;
    }
    ...
    if (std::getline(iss, line)) {
        ...
        std::cout << "Failed to read M_color_offset values."
                  << std::endl; // Debug
        return false;
    }
    ...
    if (m_values.size() != expected_m_size ||
        offset_values.size() != expected_offset_size) {
        ...
        std::cout << "Expected Offset size: " << expected_offset_size
                  << ", Actual Offset size: " << offset_values.size()
                  << std::endl;
    ...
    memcpy(this->offest.data, offset_values.data(),
           offset_values.size() * sizeof(double));

    if (!std::getline(iss, line)) {
        std::cout << "Failed to read line after M_color_offset values."
                  << std::endl;
        return false;
    }
...
double DeltaEFunction::calc(const double *x) const {
    ...
    // For Debuging, display M and Offset to ensure that the updates are
    // represented ColorManagedCalibrator m; m.display_matrix(this->M, "M");
    // m.display_matrix(this->offeset, "Offset");

    /**
     * Befor we can comput DeltaE we need to comput the x,y,z values fro each
     * color patch camera_xyz = M * (color_patch_avg - offset)
     ...
     *   (cp_avg is the average pixel value from the color target in the actual
     * image) cp_avg_chan1_patch_1, cp_avg_chan1_patch_2, ...,
     ...
     *
     *   offset_avg is a 2d Matrix computed from subtracting offset from
     * color_patch_avg cp_avg_chan1_patch_1 - offet_1, cp_avg_chan1_patch_2 -
     * offet_1, ..., cp_avg_chan1_patch_k - offet_1 cp_avg_chan2_patch_1 -
     * offet_2, cp_avg_chan2_patch_2 - offet_2, ..., cp_avg_chan2_patch_k -
     * offet_2
     *       ...                  - ...    , ...                 - ...     ,
     * ..., ...                  - ... cp_avg_chan6_patch_1 - offet_6,
     * cp_avg_chan6_patch_2 - offet_6, ..., cp_avg_chan6_patch_k - offet_6
     */

    // Create offset_avg
    cv::Mat offset_avg = btrgb::calibration::apply_offsets(
        *this->color_patch_avgs, *this->offeset);
...
void ColorManagedCalibrator::recalculateDeltaE(btrgb::ArtObject *images) {
    ...
    double deltaE_sum = btrgb::calibration::compute_deltaE_sum(
        this->ref_data, xyz, &this->deltaE_values);

    // Recalculate and update resulting_avg_deltaE
    int patch_count =
        this->deltaE_values
            .total(); // total number of elements in the deltaE matrix
