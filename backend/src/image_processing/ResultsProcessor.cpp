...
void ResultsProcessor::execute(CommunicationObj *comms,
    ...
    this->colorimetry_ver_f_name =
        this->build_output_name("ColorimetryVerification", "csv");
    this->R_camera_ver_f_name =
        this->build_output_name("R_cameraVerification", "csv");
    ...
    images->get_results_obj(btrgb::ResultType::GENERAL)
        ->store_string(PRO_FILE, this->output_dir + this->Pro_f_name);
...
void ResultsProcessor::output_btrgb_results(btrgb::ArtObject *images) {
    ...
    output_files.insert_or_assign("CM_target",
                                  this->CM_target_f_name + ".tiff");
    ...
    output_files.insert_or_assign("ColorimetryVerification",
                                  this->colorimetry_ver_f_name);
    output_files.insert_or_assign("R_cameraVerification",
                                  this->R_camera_ver_f_name);
    ...
    btrgb_json.insert_or_assign("CalibrationResults",
                                calibration_res->jsonafy());
    btrgb_json.insert_or_assign("VerificationResults",
                                verification_res->jsonafy());
...
void ResultsProcessor::output_user_results(btrgb::ArtObject *images) {
    ...
    this->write_formated_results(f_name, FormatType::M_COLOR, calibration_res,
                                 ResultObjType::CALIBRATION);
    // M_spectral
    f_name = this->M_spectral_f_name;
    this->write_formated_results(f_name, FormatType::M_SPECTRAL,
                                 calibration_res, ResultObjType::CALIBRATION);
    // Colorimetry
    calibration_res->store_matrix(CM_XYZ_REF, xyz_ref);
    f_name = this->colorimetry_f_name;
    this->write_formated_results(f_name, FormatType::COLORIMETRY,
                                 calibration_res, ResultObjType::CALIBRATION);
    // R_camera
    ...
    this->write_formated_results(f_name, FormatType::R_CAMERA, calibration_res,
                                 ResultObjType::CALIBRATION);
    ...
    if (verification_res->contains_results()) {
        ...
        this->write_formated_results(f_name, FormatType::R_CAMERA,
                                     verification_res,
                                     ResultObjType::VERIFICATION);
    }
...
void ResultsProcessor::write_formated_results(std::string file_name,
                                              FormatType format_type,
                                              CalibrationResults *results_obj,
                                              ResultObjType result_type) {
