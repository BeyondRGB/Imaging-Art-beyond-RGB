...
void CalibrationResults::write_matrix(std::ostream &output_stream,
                                      cv::Mat matrix, std::string matrix_name) {
...
void CalibrationResults::write_matrix_value(std::ostream &output_stream,
                                            cv::Mat matrix, int row, int col) {
    switch (matrix.type()) {
    case CV_8U:
...
jsoncons::json CalibrationResults::jsonafy() {
    ...
    jsoncons::json matracies =
        this->make_json_from_map<cv::Mat>(this->result_matricies);
    body.insert_or_assign(MATRIX_KEY, matracies);
    // Jsonafy the result ints
    jsoncons::json int_values =
        this->make_json_from_map<int>(this->result_ints);
    body.insert_or_assign(INT_KEY, int_values);
    // Jsonafy the result doubles
    jsoncons::json double_values =
        this->make_json_from_map<double>(this->result_doubles);
    body.insert_or_assign(DOUBLE_KEY, double_values);
    // Jsonafy the result strings
    jsoncons::json string_values =
        this->make_json_from_map<std::string>(this->results_strings);
    ...
    try {
        ...
        for (int i = 0; i < item_count; i++) {
            try {
                ...
                std::cerr << "Currupt Double Json Found" << e.what()
                          << std::endl;
            }
        }
    } catch (std::exception e) {
    ...
    try {
        ...
        for (int i = 0; i < item_count; i++) {
            try {
                ...
                std::cerr << "Currupt String Json Found" << e.what()
                          << std::endl;
            }
        }
    } catch (std::exception e) {
