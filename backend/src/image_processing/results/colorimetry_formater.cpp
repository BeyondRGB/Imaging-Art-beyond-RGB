...
void ColorimetryFormater::write_format(std::ostream &output_stream,
...
#define DELIM ","
    ...
    // Although some of them are in different formats and we will have to access
    // them differently
    ...
    for (int col = 0; col < col_count; col++) {
        for (int row = 0; row < row_count; row++) {
            ...
            // From here on out the all patch values are stored in single row so
            // we need to calculate the colum position
            ...
            // RGB_camera
            /*R_1*/ output_stream << camera_sigs.at<double>(0, mat_col)
                                  << DELIM;
            /*G_1*/ output_stream << camera_sigs.at<double>(1, mat_col)
                                  << DELIM;
            /*B_1*/ output_stream << camera_sigs.at<double>(2, mat_col)
                                  << DELIM;
            /*R_2*/ output_stream << camera_sigs.at<double>(3, mat_col)
                                  << DELIM;
            /*G_2*/ output_stream << camera_sigs.at<double>(4, mat_col)
                                  << DELIM;
...
void ColorimetryFormater::write_header(std::ostream &output_stream,
                                       std::string delim) {
    int item_count =
        sizeof(this->colum_headers) / sizeof(this->colum_headers[0]);
