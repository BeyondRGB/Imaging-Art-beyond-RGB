...
class RCameraFormater : public ResultsFormater{
    ...
    private:
        void write_header(std::ostream &output_stream, int row_count, int col_count, std::string delim);
        void write_matrix(std::ostream &output_stream, cv::Mat matrix, int row_count, int col_count, std::string delim);
