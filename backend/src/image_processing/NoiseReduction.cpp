...
void NoiseReduction::execute(CommunicationObj *comms,
    ...
    try {
        ...
        comms->send_info("Warning: No target was able to be found. Restart BeyondRGB, and reprocess with Target Images.", this->get_name());
        targets_found = false;
    }
...
void NoiseReduction::apply_filter(btrgb::Image *img1, btrgb::Image *img2) {
    ...
    cv::bilateralFilter(im1, filter1, noiseReducKernel, noiseReducKernel * 2,
                        noiseReducKernel / 2);
    cv::bilateralFilter(im2, filter2, noiseReducKernel, noiseReducKernel * 2,
                        noiseReducKernel / 2);
