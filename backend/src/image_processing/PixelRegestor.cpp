...
void PixelRegestor::execute(CommunicationObj *comms, btrgb::ArtObject *images) {
    ...
    int matched = this->appy_regestration(comms, img1, img2, 1,
                                          regestration_count, output + "img");

    if (found_target) {
        matched = this->appy_regestration(
            comms, target1, target2, 2, regestration_count, output + "target");
    }
...
int PixelRegestor::appy_regestration(CommunicationObj *comms,
