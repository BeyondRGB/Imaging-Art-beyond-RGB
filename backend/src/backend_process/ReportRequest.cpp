...
void ReportRequest::run() {
    try {
        ...
        // Need to add a way to make/adjust current CalibrationResults Obj,
        // don't have access to art obj so can't use that one currently
        this->coms_obj_m->send_reports(report_data.get_jsoncons(),
                                       "Calibration");
