...
std::shared_ptr<ImgProcessingComponent> Pipeline::pipelineSetup() {
    ...
    pre_process_components.push_back(
        static_cast<const std::shared_ptr<ImgProcessingComponent>>(
            new ImageReader()));
    // pre_process_components.push_back(static_cast<const std::shared_ptr
    // <ImgProcessingComponent>>(new ChannelSelector()));
    pre_process_components.push_back(
        static_cast<const std::shared_ptr<ImgProcessingComponent>>(
            new BitDepthScaler()));
    pre_process_components.push_back(
    ...
    if (this->get_sharpen_type() != "N") {
        pre_process_components.push_back(
            static_cast<const std::shared_ptr<ImgProcessingComponent>>(
                new NoiseReduction(this->get_sharpen_type())));
    }
    pre_process_components.push_back(
        static_cast<const std::shared_ptr<ImgProcessingComponent>>(
            new PixelRegestor(this->get_registration_type())));
    // Set up Calibration components
    std::vector<std::shared_ptr<ImgProcessingComponent>> calibration_components;
    calibration_components.push_back(
        static_cast<const std::shared_ptr<ImgProcessingComponent>>(
            new ColorManagedCalibrator()));
    calibration_components.push_back(
        static_cast<const std::shared_ptr<ImgProcessingComponent>>(
            new SpectralCalibrator()));
    if (this->should_verify) {
        calibration_components.push_back(
            std::shared_ptr<ImgProcessingComponent>(new Verification()));
    }
    calibration_components.push_back(
        std::shared_ptr<ImgProcessingComponent>(new ResultsProcessor()));

    std::vector<std::shared_ptr<ImgProcessingComponent>> img_process_components;
    img_process_components.push_back(std::shared_ptr<ImgProcessingComponent>(
        new PreProcessor(pre_process_components)));
    img_process_components.push_back(std::shared_ptr<ImgProcessingComponent>(
        new ImageCalibrator(calibration_components)));

    return std::shared_ptr<ImgProcessingComponent>(
        new ImageProcessor(img_process_components));
};
...
bool Pipeline::init_art_obj(btrgb::ArtObject *art_obj) {
    try {
        ...
        Json image_array =
            this->process_data_m->get_array(key_map[DataKey::IMAGES]);
        ...
        for (int i = 0; i < image_array.get_size(); i++) {
            ...
            try {
                std::string target_file =
                    obj.get_string(key_map[DataKey::TARGET_IMG]);
                art_obj->newImage(("target" + std::to_string(i + 1)),
                                  target_file);
            } catch (ParsingError e) {
                /* No target provided. We expect the target to be in the art
                 * image */
                this->send_info("No target provided when target was expected. "
                                "Using embedded target from art image.",
                                this->get_process_name());
            }
        }
        // Collect the information provided about the color target
        //  TargetData td;
        Json target_location =
            this->process_data_m->get_obj(key_map[DataKey::TargetLocation]);
        ...
        art_obj->setOutputFileName(
            this->process_data_m->get_string(key_map[DataKey::OUT_FILE_NAME]));
        ...
        this->report_error(
            name,
            "ParsingError Occurred while trying to initialize an ArtObject.",
...
void Pipeline::init_general_info(btrgb::ArtObject *art_obj) {
    CalibrationResults *results_obj =
        art_obj->get_results_obj(btrgb::ResultType::GENERAL);
    // Make/Model
    ...
    results_obj->store_string(GI_TARGET_ID,
                              ref_data_json.get_string("fileName"));
    // Target Dims
    ...
    // Target Coords
    results_obj->store_double(GI_TARGET_TOP,
                              target_json.get_number("top") *
                                  target_json.get_number("resolution"));
    results_obj->store_double(GI_TARGET_BOTTOM,
                              target_json.get_number("bottom") *
                                  target_json.get_number("resolution"));
    results_obj->store_double(GI_TARGET_LEFT,
                              target_json.get_number("left") *
                                  target_json.get_number("resolution"));
    results_obj->store_double(GI_TARGET_RIGHT,
                              target_json.get_number("right") *
                                  target_json.get_number("resolution"));
    // Observer
    int observer_num =
        ref_data_json.get_number(key_map[DataKey::StandardObserver]);
    results_obj->store_int(GI_OBSERVER, observer_num);
    // Illuminant
    std::string illum_str =
        ref_data_json.get_string(key_map[DataKey::Illuminants]);
...
void Pipeline::run() {
    ...
    this->send_info(this->process_data_m->to_string(),
                    this->get_process_name());
    ...
    if (lastUnderscorePosition != std::string::npos) {
        filenameWithoutExtension =
            filenameWithoutExtension.substr(0, lastUnderscorePosition);
    }
    ...
    if (batch) {
        try {
            ...
            // Use filesystem path to append filename, ensuring correct path
            // separators
            ...
            while (std::filesystem::exists(path)) {
                // Append a number to make the directory unique, using
                // filesystem to handle path
                path = parentPath / (filenameWithoutExtension + "_" +
    ...
    try {
        ...
        images.reset(new btrgb::ArtObject(ref_file, illuminant, observer,
                                          out_dir, batch));
    ...
    try {
        this->coms_obj_m->send_pipeline_components(
            pipeline->get_component_list());
        pipeline->execute(this->coms_obj_m.get(), images.get());

        std::string Pro_file = images.get()
                                   ->get_results_obj(btrgb::ResultType::GENERAL)
                                   ->get_string(PRO_FILE);
...
std::string Pipeline::get_output_directory(std::string artImage) {
    ...
    try {
        std::string base_dir =
            this->process_data_m->get_string("destinationDirectory");
        std::string dir =
            base_dir + "/" + OUTPUT_PREFIX + date_string + "_" + time_string;
        dir = dir + "/" + artImage;
        std::filesystem::create_directories(dir);
        return dir;
    } catch (ParsingError e) {
        this->report_error("[Pipeline]",
                           "Process request: The desired output directory "
                           "[\"destinationDirectory\"] is invalid.",
                           cpptrace::generate_trace());
        throw;
...
std::string Pipeline::get_sharpen_type() {
    ...
    try {
        ...
        if (sharpen_string == "H" || sharpen_string == "M" ||
            sharpen_string == "L" || sharpen_string == "N") {
            return sharpen_string;
        }
    } catch (ParsingError e) {
        this->report_error(
            "[Pipeline]",
            "Failed to parse sharpening type [" +
                this->process_data_m->get_string("sharpenString") + "].",
            cpptrace::generate_trace());
    }
    return sharpen_string;
}
...
std::string Pipeline::get_registration_type() {
    ...
    try {
        ...
        if (registration_string == "H" || registration_string == "M" ||
            registration_string == "L") {
            return registration_string;
        }
    } catch (ParsingError e) {
        this->report_error(
            "[Pipeline]",
            "Failed to parse registration string [" +
                this->process_data_m->get_string("sharpenString") + "].",
            cpptrace::generate_trace());
    }
    return registration_string;
...
TargetData Pipeline::build_target_data(Json target_json) {
    ...
    td.rotation_angle =
        target_json.get_number("calibrationTargetRotationAngle");
...
void Pipeline::init_verification(btrgb::ArtObject *images) {
    try {
        Json verification_json = this->process_data_m->get_obj(
            key_map[DataKey::VerificationLocation]);
        ...
        this->send_info("No Verification Target provided",
                        this->get_process_name());
