...
void SpectralPicker::run() {
    ...
    try {
        ...
        double x_rel =
            this->process_data_m->get_obj("coordinates").get_number("x");
        double y_rel =
        ...
        std::string local_file =
            jsoncons::json::parse(prj_file)["OutPutFiles"]["SP"]
                .as<std::string>();
        std::string filename =
        ...
        if (channels > 16)
            throw std::runtime_error("Image has more than 16 channels.");
        ...
        for (int row = 0; row < im.rows; row++) {
            for (int col = 0; col < im.cols; col++) {
                im_pixel = (uint16_t *)im.data + row * im.cols * channels +
                           col * channels;
