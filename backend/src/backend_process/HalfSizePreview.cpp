...
void HalfSizePreview::run() {
    ...
    try {
        ...
        std::unique_ptr<btrgb::LibRawReader> raw_reader(
            new btrgb::LibRawReader(btrgb::LibRawReader::PREVIEW));
        ...
        for (int i = 0; i < filenames.get_size(); i++) {
            try {
                ...
                if (im.channels() == 4) {
                    cv::Mat rg1bg2 = im;
                    im.release();
                    im.create(rg1bg2.rows, rg1bg2.cols,
                              CV_MAKE_TYPE(rg1bg2.depth(), 3));
                ...
                this->report_error(
                    ...
                    "File type unknown, or unsupported by LibRaw.",
                    cpptrace::generate_trace());
            } catch (const ParsingError &e) {
                this->report_error(this->get_process_name(), e.what(),
                                   cpptrace::generate_trace());
            } catch (const std::runtime_error &e) {
                this->report_error(this->get_process_name(),
                                   std::string(e.what()) + " (" + fname + ")",
                                   cpptrace::generate_trace());
