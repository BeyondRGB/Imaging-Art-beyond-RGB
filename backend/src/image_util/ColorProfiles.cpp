...
void ColorProfiles::convert(cv::Mat im, void *from_profile_data,
                            uint32_t from_size, void *to_profile_data,
                            uint32_t to_size) {
    ...
    colorspace_transform = cmsCreateTransform(from_profile, type, to_profile,
                                              type, INTENT_PERCEPTUAL, 0);
...
void ColorProfiles::convert_to_xyz(cv::Mat im, ColorSpace from) {
    ...
    case ColorSpace::ProPhoto:
        m = (cv::Mat_<float>(3, 3) << 0.7976749, 0.1351917, 0.0313534,
             0.2880402, 0.7118741, 0.0000857, 0.0000000, 0.0000000, 0.8252100);
        break;

    case ColorSpace::sRGB:
        m = (cv::Mat_<float>(3, 3) << 0.4360747, 0.3850649, 0.1430804,
             0.2225045, 0.7168786, 0.0606169, 0.0139322, 0.0971045, 0.7141733);
        break;

    case ColorSpace::Adobe_RGB_1998:
        m = (cv::Mat_<float>(3, 3) << 0.6097559, 0.2052401, 0.1492240,
             0.3111243, 0.6256560, 0.0632197, 0.0194811, 0.0608902, 0.7448386);
        break;

    case ColorSpace::Wide_Gamut_RGB:
        m = (cv::Mat_<float>(3, 3) << 0.7161046, 0.1009296, 0.1471858,
             0.2581873, 0.7249378, 0.0168748, 0.0000000, 0.0517813, 0.7734287);
        break;

    default:
        throw std::runtime_error(
            "[ColorProfiles::convert_to_xyz] Not implemented. ");
    }
...
void ColorProfiles::convert_to_color(cv::Mat im, ColorSpace to) {
    ...
    switch (to) {
    case ColorSpace::ProPhoto:
        m = (cv::Mat_<float>(3, 3) << 1.3459433, -0.2556075, -0.0511118,
             -0.5445989, 1.5081673, 0.0205351, 0.0000000, 0.0000000, 1.2118128);
        break;

    case ColorSpace::Adobe_RGB_1998:
        m = (cv::Mat_<float>(3, 3) << 1.9624274, -0.6105343, -0.3413404,
             -0.9787684, 1.9161415, 0.0334540, 0.0286869, -0.1406752,
             1.3487655);
        break;

    case ColorSpace::sRGB:
        m = (cv::Mat_<float>(3, 3) << 3.1338561, -1.6168667, -0.4906146,
             -0.9787684, 1.9161415, 0.0334540, 0.0719453, -0.2289914,
             1.4052427);
        break;

    case ColorSpace::Wide_Gamut_RGB:
        m = (cv::Mat_<float>(3, 3) << 1.4628067, -0.1840623, -0.2743606,
             -0.5217933, 1.4472381, 0.0677227, 0.0349342, -0.0968930,
             1.2884099);
        break;

    default:
        throw std::runtime_error(
            "[ColorProfiles::convert_to_color] Not implemented. ");
    }
...
#define BTRGB_sRGB_INVERSE_GAMMA(x)                                            \
    (x > 0.040449936 ? pow((x + 0.055) / 1.055, 2.4) : x / 12.92)
    ...
    default:
        throw std::runtime_error(
            "[ColorProfiles::inverse_gamma] Not implemented. ");
    }
...
#define BTRGB_sRGB_GAMMA(x)                                                    \
    (x > 0.0031308 ? 1.055 * pow(x, 1 / 2.4) - 0.055 : x * 12.92)
    ...
    default:
        throw std::runtime_error(
            "[ColorProfiles::apply_gamma] Not implemented. ");
    }
