...
ColorTarget::ColorTarget(btrgb::Image *im, TargetData location_data,
    ...
    // The front end normalizes the location based on width, so multiply top by
    ...
    if (this->row_count != ref_data->get_row_count() ||
        this->col_count != ref_data->get_col_count()) {
 ...
 *		sw = sp(w)
 *
 *		sw = 2sr + 1 // 1 for the center pixel and 2sr for pixels on
 * either side sr = (sw - 1) / 2
 *
 */
float ColorTarget::get_patch_avg(int row, int col, int chan) {
    ...
    double rad =
        this->rotation_angle * kPi / 180.0; // convert degrees to radians
...
/*int ColorTarget::patch_posX(int col) {
    ...
    double right = static_cast<double>(this->target_left_edge +
this->target_width) / im->width();

    // Pass variables as lvalues to transform_edges
    double top = static_cast<double>(this->target_top_edge) / im->height(); //
Needed for transform_edges double bottom =
static_cast<double>(this->target_top_edge + this->target_height) / im->height();
// Needed for transform_edges transform_edges(left, top, right, bottom,
this->rotation_angle, im->width(), im->height());
...
int ColorTarget::patch_posY(int row) {
    // Dynamically compute edges and assign them to local variables
    double top = static_cast<double>(this->target_top_edge) / im->height();
    double bottom = static_cast<double>(this->target_top_edge +
this->target_height) / im->height();

    // Pass variables as lvalues to transform_edges
    double left = static_cast<double>(this->target_left_edge) / im->width(); //
Needed for transform_edges double right =
static_cast<double>(this->target_left_edge + this->target_width) / im->width();
// Needed for transform_edges transform_edges(left, top, right, bottom,
this->rotation_angle, im->width(), im->height());
...
int ColorTarget::get_row_count() { return this->row_count; }

int ColorTarget::get_col_count() { return this->col_count; }

int ColorTarget::get_white_row() { return this->white_row; }

int ColorTarget::get_white_col() { return this->white_col; }

RefData *ColorTarget::get_ref_data() { return this->ref_data; }
