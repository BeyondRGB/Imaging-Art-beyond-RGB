...
void LibTiffWriter::_write(Image *im, std::string filename) {
    ...
    case ColorSpace::Wide_Gamut_RGB:
        throw std::logic_error(
            "[LibTiffWriter] Color profile not implemented.");
    }
    ...
    /* Write all channel data in one array (a bitmap) instead of separating the
     * channels. */
    ...
    /* The written data needs to be broken up into "Strips" to make buffering
     * easier for TIFF readers. Rows-per-strip needs to be tagged, this is the
     * number of physical pixel rows of the image written in each strip. A "row"
     * here is a bitmap row where the length is <image width> X <number of
     * channels>. A strip size of 8kB is recommended (I wonder if this is for
     * computing standars/capabilities wayback when). Anyway, since the data
     * type of the return value and estimate rows-per-strip values are uint32:
     * TIFFDefaultStripSize will not give strips that contain a fraction of a
     * row, and our image rows are more than 8kB, so the closest to the
     * recommended we can do is one row per strip. */
    /*
...
std::string LibTiffWriter::getCustomTag(Image *im) {
    ...
    case none:
        break;
    case ColorSpace::ProPhoto:
        json_tag["ColorSpace"] = "ProPhoto";
        break;
    case ColorSpace::Adobe_RGB_1998:
        json_tag["ColorSpace"] = "Adobe_RGB_1998";
        break;
    case ColorSpace::sRGB:
        json_tag["ColorSpace"] = "sRGB";
        break;
    case ColorSpace::Wide_Gamut_RGB:
        json_tag["ColorSpace"] = "Wide_Gamut_RGB";
        break;
    default:
        throw std::logic_error("[LibTiffWriter] Invalid color profile.");
    }
