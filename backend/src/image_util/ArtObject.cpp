...
ArtObject::ArtObject(std::string ref_file, IlluminantType ilumination,
                     ObserverType observer, std::string output_directory,
                     bool batch) {
...
// Collects the color target information from the request message and saves it
// in the ArtObject
...
ColorTarget ArtObject::get_target(std::string imageName, TargetType type) {
    // If a target is requested and it does not exist it must then be in the art
    // image
    ...
    try {
        ...
        if (type == TargetType::VERIFICATION_TARGET) {
            ...
            return ColorTarget(im, this->verification_data,
                               this->verification_ref);
        } else {
...
std::unique_ptr<ColorTarget>
ArtObject::get_target_pointer(std::string imageName, TargetType type) {
    try {
        Image *im = this->getImage(imageName);
        if (im == nullptr) { // Assuming getImage returns nullptr when the image
                             // doesn't exist
        ...
        if (type == TargetType::VERIFICATION_TARGET) {
            if (nullptr == this->verification_ref)
                throw ArtObj_VerificationDataNull();
            return std::make_unique<ColorTarget>(im, this->verification_data,
                                                 this->verification_ref);
        } else {
            return std::make_unique<ColorTarget>(im, this->target_data,
                                                 this->ref_data);
        }
    } catch (ArtObj_ImageDoesNotExist) {
        return nullptr; // Image not found, return nullptr
 ...
 */
void ArtObject::outputImageAs(enum output_type filetype, std::string name,
                              std::string filename) {
    ...
    try {
        if (filename == "")
            filename = name;
        ImageWriter(filetype).write(this->images[name],
                                    this->output_directory + filename);
...
std::string ArtObject::getOutputFileName() { return this->output_file_name; }
...
int ArtObject::imageCount() { return this->images.size(); }

std::string ArtObject::get_output_dir() { return this->output_directory; }

bool ArtObject::get_batch() { return this->batch; }
...
void ArtObject::init_verification_data(TargetData verification_data) {
    ...
    IlluminantType illuminant =
        RefData::get_illuminant(verification_data.illum_base);
