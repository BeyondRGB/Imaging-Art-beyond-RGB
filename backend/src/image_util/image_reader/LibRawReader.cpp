...
LibRawReader::~LibRawReader() { this->_reader.recycle(); }
...
void LibRawReader::_configLibRawParams() {
    ...
    default:
        this->_error("[LibRawReader] Invalid method.");
    }
...
void LibRawReader::copyBitmapTo(cv::Mat &im) {
    ...
    case 16:
        ...
        break;
    default:
        this->_error("[LibRawReader] Bit depth not supported.");
    }
...
static void exif_callback(void *context, int tag, int type, int len,
                          unsigned int ord, void *ifp, INT64 base) {
#define BTRGB_BUFFER_LENGTH 128

    LibRaw_abstract_datastream *data =
        static_cast<LibRaw_abstract_datastream *>(ifp);
