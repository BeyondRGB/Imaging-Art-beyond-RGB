...
void LibTiffReader::open(std::string filename) {
    ...
    if (!this->_tiff)
        throw std::runtime_error("[LibTiffReader] Failed to access file: " +
                                 filename);
    this->_is_open = true;
    ...
    if (channels == 1) {
        ...
        if (planar_config != PLANARCONFIG_CONTIG)
            throw std::runtime_error("[LibTiffReader] Only 'Chunky' planar "
                                     "config format is supported: " +
                                     filename);
    }
...
void LibTiffReader::copyBitmapTo(void *buffer, uint32_t size) {
    ...
    for (strip_index = 0; strip_index < total_strips; strip_index++) {
        strip_size = TIFFReadEncodedStrip(this->_tiff, strip_index, strip_data,
                                          strip_data_size);
        if (strip_size < 0) {
            _TIFFfree(strip_data);
            throw std::runtime_error("[LibTiffReader] Failed to read strip #" +
...
void LibTiffReader::copyBitmapTo(cv::Mat &im) {
    ...
    this->copyBitmapTo(raw_im.data,
                       _width * _height * _channels * (_depth / 8));
...
cv::Mat LibTiffReader::getCrop(uint32_t left, uint32_t top, uint32_t w,
    ...
    bool lt_valid =
        (left >= 0) && (top >= 0) && (left < _width) && (top < _height);
    bool rb_valid =
    ...
    for (row_index = top; row_index < bottom; row_index++) {
        ...
        row_size =
            TIFFReadEncodedStrip(this->_tiff, row_index, row_data, strip_size);
...
LibTiffReader::getConversionMatrices() {
    if (!this->_is_open)
        throw std::runtime_error("[LibTiffReader] No file opened.");
...
void LibTiffReader::getColorProfile(uint32_t *size, void **data) {
    ...
    if (!TIFFGetField(this->_tiff, TIFFTAG_ICCPROFILE, size, data))
        throw std::runtime_error(
            "[LibTiffReader] File does not have color profile.");
}
