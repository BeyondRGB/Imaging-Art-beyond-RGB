...
void ProcessManager::process_request(
    std::string request, std::shared_ptr<CommunicationObj> coms_obj) {
    this->set_coms_obj(coms_obj);
    std::cout << "Received: " << request << std::endl;
    std::cout << "Parsing Request" << std::endl;

    std::string RequestIDKey = key_map[ProcessManager::RequestKey::REQUEST_ID];
    std::string RequestTypeKey =
        key_map[ProcessManager::RequestKey::REQUEST_TYPE];
    std::string RequestDataKey =
        key_map[ProcessManager::RequestKey::REQUEST_DATA];
    try {
        // Pars Json
        Json j(request);
        ...
        std::thread p_thread(&ProcessManager::start_process, this, process,
                             coms_obj, request_data);
        p_thread.detach();
    } catch (ParsingError e) {
        this->report_error("ProcessManager", e.what(),
                           cpptrace::stacktrace::current());
    }
}

std::shared_ptr<BackendProcess>
ProcessManager::identify_process(std::string key) {
    ...
    if (key == "Process")
        ...
        process =
            std::shared_ptr<ColorManagedImage>(new ColorManagedImage(key));
