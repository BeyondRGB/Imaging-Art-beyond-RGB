...
int RefData::get_row_count() { return this->row_count; }

int RefData::get_col_count() { return this->col_count; }
...
WhitePoints *RefData::get_white_pts() { return this->white_pts; }
    ...
    for (int col = 0; col < col_count; col++) {
        for (int row = 0; row < row_count; row++) {
            try {
                ...
                throw RefData_ParssingError("@(" + std::to_string(row + 1) +
                                            "," + std::to_string(col + 1) +
                                            ")" + e.what());
            } catch (std::exception e) {
                std::string msg = "Row: " + std::to_string(row + 1) +
    ...
    for (int row = 0; row < this->row_count; row++) {
        ...
        for (int col = 0; col < this->col_count; col++) {
            this->color_patches[row][col] = new ColorPatch(
                row, col, this->illuminants, this->observer, this->white_pts);
        }
    }
}
...
void RefData::output_xyz() {
    ...
    std::cout << "Xn: "
              << this->white_pts->get_white_point(WhitePoints::ValueType::Xn);
    std::cout << comma << " Yn: "
              << this->white_pts->get_white_point(WhitePoints::ValueType::Yn);
    std::cout << comma << " Zn: "
              << this->white_pts->get_white_point(WhitePoints::ValueType::Zn)
              << std::endl;
    ...
    std::cout << "Y Value, " << cp->get_y() << ",Row," << cp->get_row()
              << ",Col," << cp->get_col() << std::endl
              << std::endl
              << std::endl;
}
    ...
    for (int mat_row = 0; mat_row < ref_data.rows; mat_row++) {
        ...
        for (int row = 0; row < this->row_count; row++) {
            for (int col = 0; col < this->col_count; col++) {
                ...
                // cp->get_ref_by_wavelen(wave_len) << " mat_col:" << mat_col <<
                // " wavelen: " << wave_len  << std::endl;
